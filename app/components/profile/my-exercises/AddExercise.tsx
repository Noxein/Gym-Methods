...
export const AddExercise = ({setShowModal}:{setShowModal:React.Dispatch<React.SetStateAction<boolean>>}) => {
  ...
  return (
        <BlurBackgroundModal>
            <div className={`rounded-md flex flex-col px-10 py-4 gap-4 text-xl mb-20 max-w-mobile w-mobile mx-auto text-white`} onClick={e=>e.stopPropagation()}>
                ...
                <div className='flex justify-around text-white gap-2'>
                    ...
                    <Button onClick={hideModal} className='flex-1' loading={loading}>{u("Cancel")}</Button>
                    <Button onClick={addExercise} isPrimary className='flex-1' loading={loading}>{u("Add")}</Button>
